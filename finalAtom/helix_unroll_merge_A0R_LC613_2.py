...
class Clusterer(object):
    ...
    def _find_labels(self,params):
        ...
        #dz = mm*(dz0+ii*stepdz)
        dfh['z'] = (dfh['z'] + jj) /zzi
        dfh['r'] = np.sqrt(dfh['x'].values**2+dfh['y'].values**2+(dfh['z'].values/1.5)**2)
        ...
        #hough transfer 0522
        #if(ii>60 and ii<70):
        dfh['a1'] = dfh['a0'].values - np.nan_to_num(np.arccos(dz*dfh['r'].values))
        #else:
        #dfh['a1'] = dfh['a0'].values - np.nan_to_num(np.arccos(dz*dfh['rt'].values))
        #dfh['a1'] = dfh['a0'].values +(w*dfh['z']/3000)*dz
        ...
        cx = np.array([1.5,1.5,0.73,0.4,0.02,0.02])
    ...
    def _init(self,dfhin,stage=0,newstart=1):
        ...
        #zshift_range = [4,-4,0] #good
        #zshift_range = [12,-12,8,-8,4,-4,0] #good
        #zshift_range = [15,-15,10,-10,5,-5,0] #good two
        #zshift_range = [0] 
        zshift_range = [10,-10,7,-7,4,-4,0]
        #rtzip = [0.8,0.9,1]
        #rtzip = [0.8,2,1] #good
        rtzip = [1] #good
        #zzzip = [2,1]
        ...
        #for jj in np.arange(-7.501,7.5,3):   
            ...
            for ii in tqdm(np.arange(scanuplim,scanlowlim,-1)):
...
if __name__ == "__main__":
    ...
    testsubmission = pd.concat(dataset_submissions, axis=0)
    testsubmission.to_csv('submission_train1.csv', index=False)
    ...
    if create_submission:
        for event_id, hits, cells in load_dataset(path_to_test, parts=['hits', 'cells']):
            ...
            i = i+1
            if i==5 or i%25==0:
              submission_t = pd.concat(test_dataset_submissions, axis=0)
              submission_t.to_csv('submission_600.csv', index=False)            
