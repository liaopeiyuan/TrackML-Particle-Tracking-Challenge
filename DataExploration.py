...
train = np.unique([p.split('-')[0] for p in sorted(glob.glob('/home/alexanderliao/data/Kaggle/competitions/trackml-particle-identification/train_2/**'))])
test = np.unique([p.split('-')[0] for p in sorted(glob.glob('/home/alexanderliao/data/Kaggle/competitions/trackml-particle-identification/test/**'))])
det = pd.read_csv('/home/alexanderliao/data/Kaggle/competitions/trackml-particle-identification/detectors.csv')
sub = pd.read_csv('/home/alexanderliao/data/Kaggle/competitions/trackml-particle-identification/sample_submission.csv')
