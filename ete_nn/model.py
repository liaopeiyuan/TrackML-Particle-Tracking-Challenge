import keras
from keras.layers import MaxPooling1D, Input, Dense, BatchNormalization, Dropout, PReLU, Conv1D, Reshape, Flatten
...
def complex_cnn(input_size=9):
    ...
    for layer in[
        ...
        Conv1D(kernel_size=(2), filters=100, strides=1, padding='same',
        activation="relu", kernel_initializer="glorot_normal"),
        BatchNormalization(),MaxPooling1D(2),
        Conv1D(kernel_size=(4), filters=100, strides=1, padding='same',
        activation="relu", kernel_initializer="glorot_normal"),
        BatchNormalization(),MaxPooling1D(4),
